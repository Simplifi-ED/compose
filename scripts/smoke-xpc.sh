...
echo '{"teamIDs":[],"bundleIDs":[],"allowAnySigned":true}' >"$ALLOWLIST"
chmod 600 "$ALLOWLIST"

"$COMPOSE" xpc uninstall >/dev/null 2>&1 || true
"$COMPOSE" xpc serve --binary "$XPC" &
SERVE_PID=$!
trap ' "$COMPOSE" xpc uninstall >/dev/null 2>&1 || true; kill "$SERVE_PID" 2>/dev/null || true' EXIT
sleep 2

OUTPUT="$("$SAMPLE" --mach --project demo status 2>&1)" || true
echo "$OUTPUT"
echo "$OUTPUT" | grep -q '"exitStatus"' || {
  echo "error: expected JSON status response" >&2
  exit 1
}
