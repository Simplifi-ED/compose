...
- `compose pause` / `compose unpause`: suspend/resume all project containers in place (non-destructive); `-f`, `-p`, `--profile`, `COMPOSE_PROFILES`, `--parallel`, `--dry-run`, `--machine` (workload containers in-VM, not the hypervisor VM); profile scoping same as `ps`/`down`; parallel batch via `WaveExecutionPolicy`; requires upstream `container pause`/`unpause` API (host `ContainerClient`, in-VM `container` CLI) — gateway stub until dependency bump [1].
