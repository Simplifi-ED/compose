...
- Attributes to map: `image`, `command`, `environment`, standard host-to-container `ports`, short-syntax bind-mount `volumes` (`host:container`, `host:container:ro`, comma options such as `host:container:ro,z`), short-form `depends_on` (list of service names), long-form `depends_on` with `condition: service_started` / `service_healthy` / `service_completed_successfully`, `healthcheck` (`test`, `interval`, `timeout`, `retries`, `start_period`), `init`, `profiles`, `deploy.replicas`, `deploy.resources.limits` (`cpus`, `memory`), and service-level `platform` (`linux/amd64`, `linux/x86_64` → normalized; maps to `container run --platform`; Rosetta required on Apple Silicon; plan-time error when Rosetta missing; rejects `--machine`; native `linux/arm64` omits run flag) [1].
