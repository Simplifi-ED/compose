...
- Attributes to map: `image`, `command`, `environment`, standard host-to-container `ports`, short-syntax bind-mount `volumes` (`host:container`, `host:container:ro`, comma options such as `host:container:ro,z`), short-form `depends_on` (list of service names), long-form `depends_on` with `condition: service_started` / `service_healthy` / `service_completed_successfully`, `healthcheck` (`test`, `interval`, `timeout`, `retries`, `start_period`), `init`, `profiles`, `deploy.replicas`, `deploy.resources.limits` (`cpus`, `memory`), `deploy.resources.reservations.devices` (`driver: apple`, `capabilities: [gpu]`; parse/config; plan-time runtime rejection until container GPU passthrough ships), service-level `ssh:` (`default` agent forwarding via bind-mounted `SSH_AUTH_SOCK` at `/run/ssh-auth.sock:ro`; plan-time validation of socket allowlist and reachability; missing/stale agent errors with remediation hints), `build.ssh` (`default` only — passes `--ssh default` when container build supports it; otherwise stderr warning and runtime forwarding still applies), and service-level `platform` (`linux/amd64`, `linux/x86_64` → normalized; maps to `container run --platform`; Rosetta required on Apple Silicon; plan-time error when Rosetta missing; rejects `--machine`; native `linux/arm64` omits run flag) [1].
- Profile activation: `--profile` (repeatable, OR) merged with process `COMPOSE_PROFILES` (comma-separated); applies on `up`, `down`, `pause`, `unpause`, `ps`, `logs`, `events`, `top`, `stats`, `config`, `watch` [1].
...
- `down --trim` (opt-in): guest `fstrim` during teardown on APFS hosts only — container rootfs before delete, named volumes before `down -v` removal (privileged helper), machine guest after project cleanup; failures warn, `down` still succeeds; no macOS host-root `fstrim`/`sudo`; pairs with `down -v --trim` for volumes; telemetry `disk_trim` on `OsLogTelemetry.volumes` + `SignpostTelemetry` `compose.volume` [1].
- Staged config/secret files under `~/.config/container-compose/<project>/` (`0600` secrets, `0644` configs); removed on container teardown, `down`, and pre-start failures (`ServiceRunner+HostUp` unified do/catch around prepare/parse/run; `RunSession` on parse failure) [1].
...
- Host image pull progress for `up`/`run`: pre-wave host-side pulls via `ClientImage.pull`/`unpack` with `TerminalProgress.ProgressUpdateHandler`; render compose-owned stacked stderr lines through `ImagePullProgress`; respect `--progress auto|plain|none`; suppress upstream `ContainerRun` progress on host runs when compose owns pull UI; finish pull UI before wave orchestration lines start [1].
...
- `compose events`: foreground lifecycle polling (no daemon) for project containers; default **1.5s** interval; in-memory list/status diff emits `start`/`die` to stdout; session-scoped host `ContainerClient` released on exit; `-f`, `-p`, `--profile`, `COMPOSE_PROFILES`, `--machine`, `--follow`, `--timeout`, service positional filter; profile scoping same as `logs`/`ps`; `ComposeContainerGateway.events()` reserved for future native stream (container 1.0.0 has no public API); document polling gaps (sub-second containers may miss events; no `oom`/health) [1].
- `compose top` / `compose stats` (alias): live CPU/memory/network/block I/O table via `ContainerClient.stats()`; `stats` is an ArgumentParser alias on `Top`; `--interval SECONDS` (minimum 1, default 2) between refreshes; TTY redraw + plain/pipe modes (pipe = single snapshot); project + profile filtering; rejects `--machine` [1].
- Host DNS (`up --host-dns`, macOS host only): service-level `x-compose.hosts` maps declared hostnames to `127.0.0.1` (NAT) or container bridge IP (bridged networks) via delimited `/etc/hosts` blocks (`# BEGIN container-compose:<project>:<projectID>`); **single install after startup waves** (one admin prompt); ownership JSON in user `~/.config/container-compose/<project>/host-dns.json`; privileged apply via macOS admin prompt only (never run compose under `sudo`); `down` removes mappings best-effort with manual remediation on elevation cancel; rejects `--machine`; config emits warnings (not errors) for invalid declarations unless `--host-dns` [1].
- Guest clock sync on macOS wake (host only): process-scoped `NSWorkspace.didWakeNotification` observer while a compose CLI runs; reconciles compose-labeled running containers (host sandbox via `ContainerClient.dial` + `Vminitd.setTime`; machine mode syncs hypervisor VM when in-VM compose workloads exist); eager sync on runtime command entry and one-shot after `up` before detach; opt-out `COMPOSE_CLOCK_SYNC=0` or `--no-clock-sync`; failures warn on stderr, never crash CLI; lifecycle telemetry `clock_sync_*` on `OsLogTelemetry.lifecycle` (counts only, no container IDs); see `docs/clock-sync-spike.md` and `docs/clock-sync-manual-test.md` [1].
- `--machine <name>` on `up`/`down`/`pause`/`unpause`/`exec`/`ps`/`logs`/`events`/`cp`: validate machine via `MachineClient.inspect` (lazy boot); `ensureBooted()` passes host `SSH_AUTH_SOCK` in `dynamicEnv` at boot and workload `container run` mounts the agent socket for services with `ssh: [default]` (Branch A — same mount flags as host sandbox); scope discovery with `com.docker.compose.machine` label; in-VM `container` CLI via `MachineInVMRunner` for list/run/stop/exec/cp/logs/build; host sandbox discovery excludes machine-labeled containers; `ps`/`logs`/`events` exit gracefully when machine stopped; execution banner on stderr; dry-run prefixes `machine=<name>` without booting; `watch`/`run`/`top`/`stats`/`config` error on `--machine` [1].
- Minimal compose `networks:`: root declarations (empty/no-option form; `external: false` only; `x-compose.network.mode: bridge` or `driver: bridge` sugar for bridged attachment) and service memberships (short list + empty-map long form); project-scoped network create (`{project}_{network}`) before startup waves and removal on `down`; attach via `container run --network`; DNS resolves **container names** (`{project}_{service}_{index}`), not Docker-style service shorthand; custom networks require macOS 26+; bridge create gated on upstream `container` API (`ComposeError.bridgeNetworksUnsupported` until shipped); `compose ps` IP column from `ContainerSnapshot.networks`; see `docs/bridged-network-spike.md` [1].
...
- Unified Logging telemetry: structured `os.Logger` in `ComposeCore/Telemetry/` (subsystem `com.simplifi-ed.container-compose`; categories `orchestration`, `lifecycle`, `signals`, `volumes`, `networks`, `build`); high-signal orchestration events at gateway/runner call sites; Instruments interval signposts via `SignpostTelemetry` (`compose.parse`, `compose.plan`, `compose.execute.wave`, `compose.discovery`, `compose.network`, `compose.volume`); opt-out via `COMPOSE_OSLOG=0` or `--no-oslog` (disables logs and signposts); no telemetry during `--dry-run`; never log secrets, env values, build args, or staged config/secret paths; supplements stderr/stdout (does not replace) [1].
- XPC automation API (additive): `ComposeCore/XPC/` exposes `ComposeXPCProtocol` (`status`/`ps`/`up`/`down`) over NSXPC Mach service `com.simplifi-ed.container-compose.xpc` with JSON payloads; `compose-xpc` helper + `compose xpc serve|install|uninstall|doctor`; client auth via `SecCode` (PID guest) + `~/.config/container-compose/xpc-clients.json` allowlist (`allowAnySigned` dev-only); rejects unsigned clients; `up` requires explicit `files[]`; no secret/env leakage in replies; path validation via `ComposePathValidation`; see `docs/xpc-clients.md` and `docs/xpc-architecture-spike.md`; remote/network RPC and anonymous IPC out of scope [1].
...
- Load project skills under `.agents/skills/` (especially `swift-concurrency`, `swift-testing-pro`, `writing-for-interfaces`, `swift-format-style`, `ios-code-audit`) when implementing features; delete root `CODE_AUDIT.md` once actionable ios-code-audit items are implemented.
...
- Deliver merge-ready PRs with polish included—not deferred follow-up cleanup; address valid CodeRabbit PR review hardening findings alongside Thermos passes.
- Run Thermos parallel review (`thermo-nuclear-review` + code-quality subagents) before declaring feature work complete or filing GitHub issues; fix all findings (major, minor, deferrable).
- CI/release automation targets this compose plugin repo only—not upstream `apple/container`; `pr.yml` validates fork PRs (read-only token, lint/build/test/`make verify-codesign`); keep `.github/workflows/ci.yml` as a maintainer-run quality gate (`workflow_dispatch`), not per-commit, to conserve macOS runner minutes; draft pre-release changelogs as gitignored markdown under `dist/` (e.g. `dist/v0.0.1-rc2.md`) for `gh release --notes-file`; Homebrew formula bumps belong only in [`Simplifi-ED/homebrew-compose`](https://github.com/Simplifi-ED/homebrew-compose)—never commit `Formula/` or tap mirror files in the compose repo (`release.yml` `bump-homebrew-formula` checks out the tap, sets explicit `version` from tag, and pushes there).
- Apply ponytail (lazy senior dev: YAGNI, deletion over addition, no unrequested abstractions) on all implementation work—not only when explicitly reminded.
...
- Package targets: `ComposeCore`, `compose`, `compose-verify`; on CLT use `swift run -c release compose-verify` (not `swift test`) and `scripts/lint.sh` with `SWIFTLINT_DISABLE_SOURCEKIT=1`. `compose-verify` sync bridge: `blockingAwait` runs async work via `Task.detached`; `StandardStreamCapture.captureStandardError` restores stderr before reading the pipe (avoids deadlock)—use for tests that intentionally emit production stderr. `TestRunner+Ps` `TableFormat` rows must match live `compose ps` column count (5 with IP); a 4-cell row against a 5-column table triggers SIGTRAP in debug verify (exit 133, often mistaken for a container/runtime crash). In `expect()` assertions, avoid bare array subscript (no short-circuit)—use `.first`/bounds checks so failures record cleanly instead of trapping. CI: `pr.yml` (lint/debug build/test/`make verify-codesign`; `protect-main` ruleset requires `validate`); `ci.yml` manual-only via `workflow_dispatch` with opt-in `run_smoke`; `release.yml` on version tags. Release `entitlements.plist` is capability-only (`hypervisor`, `network.client`)—no `app-sandbox`/`network.server`/`files.user-selected` until Phase 0 spike (`docs/entitlements-audit.md`). `.github/CODEOWNERS`: `@Simplifi-ED/dev` default, `@Simplifi-ED/admin` for CI/scripts/Makefile/Package.swift/AGENTS.md. Dependabot weekly on `github-actions`. SwiftLint via `scripts/install-swiftlint.sh` before `make lint`.
...
- Compose file loading: default discovery `compose.yaml` → … → `docker-compose.yml` + paired `*.override.*`; `COMPOSE_FILE` when no `-f`. Project name: `-p` → `COMPOSE_PROJECT_NAME` → merged `name:` → first-file parent dir. Explicit missing `-f` paths throw; only absent default discovery yields nil so `-p`-only `down`/`ps` can proceed. Multi-file `-f` merge and `include:` via `ComposeIncludeResolver`—unique-key ports/volumes; env by var name; `depends_on` keyed merge; `healthcheck` override wins; `deploy.resources.limits` per-field merge; `deploy.resources.reservations` whole-block override-wins; `build.ssh` field-level within merged build block (override non-empty wins, empty preserves base); relative bind-mount paths against compose file directory; parent services win on `include:` (duplicate errors); cycle rejection; per-include `env_file`/`project_directory`.
...
- Compose `networks:` via `NetworkPlanning`/`NetworkRunner`: validate in `validate()` only; create `{project}_{network}` before startup waves; `down` removal scoped to teardown∪running services (same model as volume purge) and throws on invalid scoped config; host/machine create-delete is project-label scoped (warn on unlabeled reuse). Merge `-f`: bare `backend:` under service `networks:` is YAML null (removes membership)—use `backend: {}` for empty-map membership. Bridged attachment via `x-compose.network.mode: bridge` or `driver: bridge`; bridge create throws `ComposeError.bridgeNetworksUnsupported` until upstream `container` ships bridge network API (apple/container#1463). Per-container IP from `status.networks` via `ContainerNetworkDiscovery` (bridge-first, DHCP retry); bridged `--host-dns` installs once after startup via `HostDNSMapping.installAfterStartup`. Live checks: `make smoke-networks` + `make smoke-networks-bridge` (macOS 26+; bridge smoke skips when upstream bridge unavailable). See `docs/bridged-network-spike.md`.
