...
- CLI subcommands: `up`, `down`, `ps`, `logs`, `top`, `exec`, `cp`, `run`, `watch`, `config`; shared flags via `@OptionGroup` `ParsableArguments` structs (`ProjectOptions`, `ProfileOptions`, …) with synthesized `Codable`—delegate to pure resolution helpers, not `ResolutionCache`/custom `init(from:)` unless encoding is required; `up`/`down` accept `--progress auto|plain|none`; `up --attach` multiplexes logs after detached start; `-f`/`-p` on subcommands, not `compose` root.
