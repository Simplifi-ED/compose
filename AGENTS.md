...
- Profile activation: `--profile` (repeatable, OR) merged with process `COMPOSE_PROFILES` (comma-separated); applies on `up`, `down`, `pause`, `unpause`, `ps`, `logs`, `events`, `top`, `config`, `watch` [1].
