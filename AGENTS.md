...
- Unified Logging: `ComposeCore/Telemetry/OsLogConfiguration` + `OsLogTelemetry` + `SignpostTelemetry` (subsystem `com.simplifi-ed.container-compose`); `OsLogConfiguration.apply` at command entry; process env `COMPOSE_OSLOG=0` and `--no-oslog` (`OsLogOptions`) opt out of logs and Instruments signposts; dry-run suppresses all os_log/signposts; secrets/env/build args never logged.
