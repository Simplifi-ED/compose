...
- Profiles: default `up` skips profiled services; `--profile` (repeatable, OR) and `COMPOSE_PROFILES` (comma-separated, union with CLI) on `up`, `ps`, `logs`, `top`, `down`, `config`, `watch`; `down --profile "*"` or `COMPOSE_PROFILES=*` stops all project containers; `run` auto-enables the target service's profiles; filter before dependency graph with `profileExcludedDependency` for inactive deps.
