...
- CLI subcommands: `up`, `down`, `ps`, `logs`, `top`, `exec`, `run`, `watch`, `config`; `up`/`down` accept `--progress auto|plain|none`; `up --attach` multiplexes logs after detached start; `-f`/`-p` on subcommands, not `compose` root.
...
- `develop.watch` via `compose watch`: FSEvents (`FileWatchMonitor`), debounced sync (`WatchDebouncer` + `ContainerFileSync.copyIn`), `sync+restart` via `ServiceRunnerRestart.restartPlans` (all replicas, no project teardown); path sandbox reuses `BindMountPathResolver`; Ctrl+C uses `.cancelOnly`.
